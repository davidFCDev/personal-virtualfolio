...
const ToggleSound = () => {
    ...
    return (
        <div className='fixed z-20 bottom-4 right-4 flex flex-row-reverse text-white gap-1'>
            ...
            <button onClick={handleMusicNoteClick} className='flex flex-row-reverse items-center gap-3 relative'>
                <MdMusicNote className='hover:scale-110 text-2xl'/>
                {showTitle && (
                    <p
                        className={`text-[15px] italic ${showTitle ? 'color-transition' : ''}`}
            ...
            {soundEnabled && currentAudio && <audio autoPlay src={currentAudio} onEnded={handleAudioEnded}/>}
        </div>
    );
};
